...
@Component
public class FullImagePersistenceHandler implements ImagePersistenceHandler {
    private static final Logger logger = LoggerFactory.getLogger(FullImagePersistenceHandler.class);
    ...
    public Future<ImageSavedSummaryByType> save(UUID globalId, MultipartFile image) {
        return Future.of(() -> {
            logger.info("Saved {} multipart file as {}!", globalId, ImageType.FULL);
            return new ImageSavedSummaryByType(ImageType.FULL, Try.success(false));
        });
    }
}
