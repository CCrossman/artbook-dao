...
@Component
public class TwitterCardImagePersistenceHandler implements ImagePersistenceHandler {
    private static final Logger logger = LoggerFactory.getLogger(TwitterCardImagePersistenceHandler.class);
    ...
    public Future<ImageSavedSummaryByType> save(UUID globalId, MultipartFile image) {
        return Future.of(() -> {
            logger.info("Saved {} multipart file as {}!", globalId, ImageType.TWITTER);
            return new ImageSavedSummaryByType(ImageType.TWITTER, Try.success(false));
        });
    }
}
